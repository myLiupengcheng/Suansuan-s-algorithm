...
/**
 * 选择排序相关算法，主要使用原理如下所示： 1、假设存在4个无序数字组成的数组，分别[3,2,7,1] 2、从 0～3
 * 的位置上面找到一个最小的数字，将该数字放置到第 0 位上面,比如找到就是 1，将 1 这个数字放置到 0 的位置上面 3、从 1～3
 * 的位置上面找到一个最小的数字，将该数字放置到第 1 位上面,比如找到就是 2，将 2 这个数字放置到 1 的位置上面 4、从 2～3
 * 的位置上面找到一个最小的数字，将该数字放置到第 2 位上面,比如找到就是 3，将 3 这个数字放置到 2 的位置上面 5、从 3～3
 * 的位置上面找到一个最小的数字，将该数字放置到第 3 位上面,比如找到就是 7，将 7 这个数字放置到 3 的位置上面 . . .
...
public class Algorithm_SelectionSort {
	...
	public static void main(String[] args) {
		...
		selectionSort(ArrayUtils.ARRAY);
